...
As our lives become more and more complex, so does the underlying science behind what makes that complexity possible. All of this complexity is based on lengthy calculations, models, code, physics and algorithms. 
Each separate individual being has its own capabilities of understanding and living in this complexity we call life. Every being therefore forms its own reality. Set of parameters and assumptions that are underlying what it thinks and does. But the only thing that connects all of those realitys is mathematics.

As bees, birds, humans and many creatures understand at least basic mathematics, so does the universe follow the sets of observable truth. And as we build a good enough understanding of them to build our own future with technology and science, not only do our realities become more mathematical, so do we understand more and more of the nature of things that came before it.

Modern Pythagoreanism is therefore a believe, that everything can be and will be explained in sufficient enough mathematics. Not only the statistically possible future, but us, the beginning and the end of the universe that we live in. And all our realities will be corrected and built with mathematics. Collecting us into one reality with one indisputable underlying truth - mathematics.
